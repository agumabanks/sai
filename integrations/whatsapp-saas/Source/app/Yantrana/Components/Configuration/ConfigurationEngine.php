...
class ConfigurationEngine extends BaseEngine implements ConfigurationEngineInterface
{
    ...
    public function processConfigurationsStore($pageType, $inputData, $ignoreOtherFields = false)
    {
        ...
        $embeddedSignUpAddon = true; // (getAppSettings('lwAddonWhatsJetEmbeddedSignUpAddon', 'registration_id') and (sha1(array_get($_SERVER, 'HTTP_HOST', '') . getAppSettings('lwAddonWhatsJetEmbeddedSignUpAddon','registration_id') . '1.0+') === getAppSettings('lwAddonWhatsJetEmbeddedSignUpAddon','signature')));
        $isExtendedLicense = true; // (getAppSettings('product_registration', 'licence') === 'dee257a8c3a2656b7d7fbe9a91dd8c7c41d90dc9');
        ...
        if (!__isEmpty($inputData)) {
            ...
            foreach ($defaultSettings as $defaultInputKey => $defaultInputValue) {
                ...
                if (
                    Arr::get($defaultSettings, "$inputKey.ignore_empty") and (!$inputValue and (!Str::startsWith($inputKey, [
                        'enable_',
                        'allow_'
                    ])))
                ) {
                ...
                if (
                    !$embeddedSignUpAddon and in_array($inputKey, [
    ...
    {
        return $this->engineSuccessResponse(['show_message' => true], __tr('Registration successful.'));
    }
    ...
    {
        return $this->engineSuccessResponse(['show_message' => true], __tr('Registration removed.'));
    }
    ...
    public function processInstallAddon($request)
    {
        ...
        if ($zip->open($filePath) === true) {
            ...
            if ($containsMetadata and $addonMetadataFileContent) {
                // Safely extract the identifier using regex instead of executing the PHP file
                if (preg_match("/(['\"])identifier(['\"])\s*=>\s*(['\"])([^'\"]+)(['\"])/", $addonMetadataFileContent, $matches)) {
                    $addonMetadata = [
                ...
                if ($containsMetadata and isset($addonMetadata['identifier'])) {
